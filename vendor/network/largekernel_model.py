...
class largekernelseg(nn.Module):
    def __init__(self, config):
        ...
        self.rwkv_layers = config['model_params']['rwkv_layers']
    ...
    """
    def forward(self, data_dict):
        # Move no_grad outside if possible (better for compile)
        data_dict = self.voxelizer(data_dict)

        data_dict = self.voxel_3d_generator(data_dict)

        # Avoid Python list append in loop
        #enc_feats = tuple(
        #    enc(data_dict) for enc in self.spv_enc
        #)
        enc_feats = []
        for i in range(self.num_scales):  # NOT self.num_scales if that can drift
            enc_feats.append(self.spv_enc[i](data_dict))

        # cat over static tuple is easier for compiler
        output = torch.cat(enc_feats, dim=1)

        logits = self.classifier(output)

        # Avoid mutating dict in-place if possible
        return {**data_dict, "logits": logits}
