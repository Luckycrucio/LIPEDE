...
try:
  ...
  #os.environ['PYTHONPATH'] = os.environ.get('PYTHONPATH', '') + ':/home/fusy/Documents/DL/point_cloud_suite/LSK3DNet/c_utils/build'
  from c_gen_normal_map import gen_normal_map
  USING_NATIVE_NORMALS = True

  # import ctypes
  # from ctypes import cdll
  # gen_normal_map = cdll.LoadLibrary('c_utils/build/c_gen_normal_map.cpython-37m-x86_64-linux-gnu.so').gen_normal_map
except ImportError:
  USING_NATIVE_NORMALS = False

  def gen_normal_map(current_range, current_vertex, proj_H=64, proj_W=900):
    """Vectorized fallback for the optional pybind11 normal-map extension."""
    points = current_vertex[..., :3]
    right = np.roll(points, -1, axis=1)
    below = np.roll(points, -1, axis=0)
    valid = current_range > 0
    valid &= np.roll(current_range, -1, axis=1) > 0
    valid &= np.roll(current_range, -1, axis=0) > 0
    valid[-1, :] = False
    u = right - points
    v = below - points
    u_norm = np.linalg.norm(u, axis=2, keepdims=True)
    v_norm = np.linalg.norm(v, axis=2, keepdims=True)
    u = np.divide(u, u_norm, out=np.zeros_like(u), where=u_norm > 0)
    v = np.divide(v, v_norm, out=np.zeros_like(v), where=v_norm > 0)
    cross = np.cross(v, u)
    norm = np.linalg.norm(cross, axis=2, keepdims=True)
    normals = np.full((proj_H, proj_W, 3), -1, dtype=np.float32)
    usable = valid & (norm[..., 0] > 0)
    normals[usable] = (cross / np.maximum(norm, 1e-12))[usable]
    return normals
